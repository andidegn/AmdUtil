...
namespace AMD.Util.View.WPF.UserControls.Chart
{
  ...
  internal class TopCanvasValueScaleConverter : IMultiValueConverter
  {
    public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
    {
      ...
      {
        double totalHeight = vt.Item2.ActualHeight;
  ...
  public partial class AmdChart : UserControl
  {
    ...
    {
      canvasLineData.InvalidateVisual();
      //canvasLineData.UpdateLayout();
      ...
      {
        double longestString = ItemsSource.Select(x => x.TBXAxisValueName?.ActualWidth ?? 0).Max();
        double colWidth = gridXAxisNames.ActualWidth / gridXAxisNames.ColumnDefinitions.Count;
        int skipCount = (int)(longestString / colWidth * 2 + 0.5);
        if (0 < skipCount)
        {
    ...
    private void AddLine(LineChartValue previous, LineChartValue current, Panel parent)
    {
      if (/*null != previous && */null != current && null != parent)
      {
        ...
        canvasLineData.Lines.Add(new LineChartValueLocal(current, parent));
      }
    }
    #endregion // Shapes
      ...
      if (sender is ScrollViewer)
      {
        ...
        else
        {
          if (Modifier.IsAltDown && Modifier.IsCtrlDown)
          {
            ...
            if (double.IsNaN(YAxisZoomHeight) || YAxisCurrentMax * 20 > YAxisZoomHeight)
            {
