...
namespace AMD.Util.Colour.Devcorp
{
  ...
  public sealed class ColorSpaceHelper
  {
    ...
    /// <param name="green">Green must be in [0, 255].</param>
    /// <param name="blue">Blue must be in [0, 255].</param>
    public static CIEXYZ RGBtoXYZ_sRGB_D65(int red, int green, int blue)
    {
      // normalize red, green, blue values
      double rLinear = (double)red / 255.0;
      double gLinear = (double)green / 255.0;
    ...
    /// Converts RGB to CIEXYZ.
    /// </summary>
    public static CIEXYZ RGBtoXYZ_sRGB_D65(RGB rgb)
    {
      return RGBtoXYZ(rgb.Red, rgb.Green, rgb.Blue);
    }
    /// <summary>
    /// Converts RGB to CIEXYZ.
    /// </summary>
    public static CIEXYZ RGBtoXYZ_sRGB_D65(Color c)
    {
      return RGBtoXYZ(c.R, c.G, c.B);
    }
    ...
    /// Converts CIEXYZ to RGB structure.
    /// </summary>
    public static RGB XYZtoRGB_sRGB_D65(double x, double y, double z)
    {
      double[] Clinear = new double[3];
    ...
    public static double[,] InverseTransformationMatrix { get; set; }

    public static RGB XYZtoRGBCustom(double x, double y, double z)
    {
      return XYZtoRGB(x, y, z, InverseTransformationMatrix);
    }

    /// <summary>
    /// Converts CIEXYZ to RGB structure.
    /// </summary>
    public static RGB XYZtoRGB(CIEXYZ xyz, double[,] inverseTransformationMatrix)
    {
      return XYZtoRGB(xyz.X, xyz.Y, xyz.Z, inverseTransformationMatrix);
    }
    ...
    /// <returns></returns>
    public static RGB XYZtoRGB(double x, double y, double z, double[,] itm)
    {
      double[] Clinear = new double[3];
      Clinear[0] = x * itm[0, 0] + y * itm[0, 1] + z * itm[0, 2]; // red
      Clinear[1] = x * itm[1, 0] + y * itm[1, 1] + z * itm[1, 2]; // green
      Clinear[2] = x * itm[2, 0] + y * itm[2, 1] + z * itm[2, 2]; // blue
    ...
    /// Converts CIEXYZ to RGB structure.
    /// </summary>
    public static RGB XYZtoRGB_sRGB_D65(CIEXYZ xyz)
    {
      return XYZtoRGB_sRGB_D65(xyz.X, xyz.Y, xyz.Z);
    }
